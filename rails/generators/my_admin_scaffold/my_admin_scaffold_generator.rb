class MyAdminScaffoldGenerator < Rails::Generator::NamedBase
  ...
  def manifest
    record do |m|
      ...
      m.template "view_form.html.erb", File.join('app/views/admin', controller_class_path, controller_file_name, "_form.html.erb")
      m.template(
        'controller.rb', File.join('app/controllers/admin', controller_class_path, "#{controller_file_name}_controller.rb")
      )
      
      m.template(
        'public_controller.rb', File.join('app/controllers', controller_class_path, "#{controller_file_name}_controller.rb")
      )
  ...
  def route_my_resources(*resources)
    ...
    unless options[:pretend]
      gsub_file 'config/routes.rb', /(#{Regexp.escape(sentinel)})/mi do |match|
        "#{match}\n  map.resources #{resource_list}, :only => [:index, :show]\n"
      end
    end
=begin
    resource_list = resources.map { |r| r.to_sym.inspect }.join(', ')
    sentinel = 'ActionController::Routing::Routes.draw do |map|'
    logger.route "map.resources #{resource_list}, :member => { :confirm_destruction => :get }"
    unless options[:pretend]
      gsub_file 'config/routes.rb', /(#{Regexp.escape(sentinel)})/mi do |match|
        "#{match}\n  map.resources #{resource_list}, :member => { :confirm_destruction => :get }\n"
      end
    end
=end
  end
